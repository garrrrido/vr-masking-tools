...
def read_frame_from_videos(frame_root):
    if frame_root.endswith(VIDEO_EXTENSIONS):  # Video file path
        ...
        if not cap.isOpened():
            raise RuntimeError(f"Failed to open video: {frame_root}")
        ...
        while True:
            ...
            if not ret:
                break
            frames_list.append(frame[..., ::-1].copy())  # BGR→RGB, HWC
        cap.release()

        frames = torch.from_numpy(np.stack(frames_list)).permute(0, 3, 1, 2).contiguous()  # TCHW

        # calculate actual fps from frame count and duration, not metadata
        frame_count = frames.shape[0]
        if meta_fps > 0 and frame_count > 0:
            duration = frame_count / meta_fps
            fps = frame_count / duration  # same as meta_fps when valid
        else:
            fps = meta_fps if meta_fps > 0 else 24.0
    else:
